...
const EJECT_DECEL = 0.086; // velocity retention per second (≈ 0.96^60 — now dt-scaled)
...
class EjectedMass {
  ...
  update(dt) {
    this.x += this.vx * dt;
    this.y += this.vy * dt;
    var decel = Math.pow(EJECT_DECEL, dt);
    this.vx *= decel;
    this.vy *= decel;
...
class Bot {
  ...
  kill() {
    this.alive = false;
    this.respawnTimer = 2 + Math.random() * 3; // 2–5 s before respawning
  }
...
class Particle {
  ...
  update(dt) {
    this.x += this.vx * dt;
    this.y += this.vy * dt;
    var drag = Math.pow(0.046, dt); // 0.95^60 per-second — now dt-scaled
    this.vx *= drag;
    this.vy *= drag;
    this.life -= dt;
  }
...
class Game {
  constructor() {
    ...
    // Start loop — bind once so RAF callbacks don't close over the global
  ...
  respawnBots(dt) {
    for (var i = 0; i < this.bots.length; i++) {
      ...
      if (!bot.alive) {
        bot.respawnTimer -= dt;
        if (bot.respawnTimer <= 0) bot.respawn();
      }
    }
  }
  ...
  drawCell(ctx, cam, cell) {
    ...
    var points = Math.max(20, Math.min(64, Math.floor(sr * 0.8))); // cap at 64 for perf

    ctx.beginPath();
    for (var i = 0; i < points; i++) { // i < points: avoid duplicating start vertex
  ...
  drawHUD() {
    ...
    for (var i = 0; i < top.length; i++) {
      ...
      html += '<li' + cls + '>' + (i + 1) + '. ' + escapeHTML(top[i].name) + '</li>';
    }
    list.innerHTML = html;
  }
